...
│                                                                    │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                 pi-evolver extension                         │   │
│  │         ~/.pi/agent/extensions/evolver.ts                    │   │
│  │                                                              │   │
│  │  session_start        ─ init session stats                   │   │
│  │  before_agent_start   ─ read digest.md, append to sys prompt │   │
│  │  tool_execution_start ─ remember call start time             │   │
│  │  tool_result          ─ count calls, detect errors, hash sig │   │
│  │  session_shutdown     ─ derive outcome, append to events     │   │
│  │  /evolver (command)   ─ on-demand digest view                │   │
│  └─────────────────────────────────────────────────────────────┘   │
└────────────────────────┬───────────────────────────────────────────┘
                  ...
                  │   digest.md           ←── written     │
                  │   digest.json         ←── written     │
                  │   personality.json    ←── written     │
                  │   evolver.log         ←── appended    │
                  │   DISABLED            ←── kill switch │
                  │   lib.mjs, daemon.mjs, pi-evolver     │
                  └──────┬────────────────────┬──────────┘
                         │                    │
                         │ reads events       │ reads digest
                         ▼                    │
     ┌──────────────────────────────────┐    │
     │  daemon.mjs (one-shot)            │    │
     │                                   │    │
     │  - rotateEventsIfNeeded()         │    │
     │  - readRecentEvents(500)          │    │
     │  - analyzeEvents()                │    │
     │  - writeDigest()                  │    │
     │    └─ updatePersonality()         │    │
     └───────────┬───────────────────────┘    │
     ...
     │    com.pi.evolver.plist           │     │
     │                                   │     │
     │  RunAtLoad + StartInterval=21600s │     │
     └───────────────────────────────────┘     │
                                               │
     ┌─────────────────────────────────────────┴───┐
     │  pi-evolver CLI                              │
     │                                              │
     │  status / run / tail / promote / personality │
     │  disable / enable / install-launchd / …      │
     └──────────────────────────────────────────────┘
```
...
```
if (consecutiveFailures >= 3)     → innovate
else if (consecutiveEmpty >= 3)   → steady-state
else if (saturatedSignals > 0)    → harden
else if (total_events <= 5)       → early-stabilize
else                              → balanced
```
